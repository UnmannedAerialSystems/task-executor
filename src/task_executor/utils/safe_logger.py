...
def configure_logging(level=logging.INFO) -> logging.Logger:
    ...
    """
    # Ensure the task-executor-logs directory exists
    os.makedirs("./task-executor-logs", exist_ok=True)

    # Create a log file with a timestamp
    log_filename = f"./task-executor-logs/log_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.txt"
